...
```yaml
# Example: vllm_omni/model_executor/stage_configs/qwenimage_cachedit.yaml
stage_args:
  - stage_id: 0
    runtime:
      process: true
    ...
    engine_args:
      ...
      backend_config:         #  DIFF: parse via stage_config?
        pipeline_config: "{torch_dtype: bfloat16}"
...
> 实际使用中，我们建议按照 `{model_name}_{backend}.yaml` 将配置拆分，比如
> `qwenimage_diffusers.yaml`、`qwenimage_cachedit.yaml`，便于针对不同组合
> 维护独立的 `pipeline_config` / `backend_config`。
...
```python
# vllm_omni/model_executor/models/diffusers_pipeline_generator.py
from diffusers import DiffusionPipeline

class DiffusersPipelineGenerator:
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        pipeline_kwargs = dict(
            vllm_config.model_config.backend_config.get("pipeline_config", {})
        )
        self.pipeline = DiffusionPipeline.from_pretrained(
            vllm_config.model,
            torch_dtype=pipeline_kwargs.pop(
                "torch_dtype", vllm_config.model_config.dtype
            ),
            trust_remote_code=vllm_config.trust_remote_code,
            **pipeline_kwargs,
        )
        backend = vllm_config.model_config.model_backend
        backend_config = vllm_config.model_config.backend_config
        if backend == "cache-dit":
            cache_dit.enable_cache(
                self.pipeline,
                cache_config=DBCacheConfig(**backend_config.get("cache_config", {})),
                parallelism_config=ParallelismConfig(
                    **backend_config.get("parallel_config", {})
                ),
            )
